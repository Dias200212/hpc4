...
int main(int argc, char **argv) {
    ...
    MPI_Init(&argc, &argv); //the system passes some starting parameters.
    ...
    MPI_Comm_rank(MPI_COMM_WORLD, &rank); //stores a specific process number at an address
    MPI_Comm_size(MPI_COMM_WORLD, &size); //communicator and number of threads processing the program
    ...
    int block_size = mySizeY/size; // splitting an array to blocks with dividing an array size to number of blocks
    int block_start = rank * block_size;
    int last_rank = size-1;
    if (rank==last_rank){
        block_size = block_size + (mySizeY%size);
        }
    // Each process stores its piece only.
    Image myPiece(mySizeX, block_size);
    
    double ts = MPI_Wtime(); //Returns an elapsed time on the calling processor
    ...
    MPI_Isend(myPiece.getData(), mySizeX * block_size * 3, MPI_DOUBLE, 0, 0, MPI_COMM_WORLD, &send_request);//blocking message transmission

    // The main process will gather the final (resulting) image.
    if (rank == 0) { //let's consider that rank 0 would collect final image
	    ...
	    if (src_rank == last_rank){
            srcSizeY = srcSizeY + (mySizeY%size);}
            // Allocating temporary space to receive an image piece from src_rank process.
            Image srcPiece(srcSizeX, srcSizeY);
            // Receiving a piece.
            MPI_Recv(srcPiece.getData(), srcSizeX * srcSizeY * 3, MPI_DOUBLE, src_rank, 0, MPI_COMM_WORLD, MPI_STATUS_IGNORE); // receiving a message
    ...
    MPI_Finalize(); //completion of the combination of parts of the program.
